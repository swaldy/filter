...
if __name__ == "__main__":
    ...
    if mtype == "keras":
        ...
        model = CreateModel(shape, nb_classes, first_dense, model_file = config["keras_model_file"])
    ...
    if mtype == "qkeras":
        ...
        model = CreateQModel(shape, model_file=model_file)
        # generate hls model
        ut.gen_hls_model(model, output_dir=outDir)
    ...
    if dataset == "test":
        x_test, y_test = ut.loadExampleTestVectors()
    ...
    np.save(os.path.join(outDir, f"{mtype}_predictions.npy"), predictions)
